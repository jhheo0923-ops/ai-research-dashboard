...
- arXiv `cs.AI`, `cs.LG`, `cs.CL`, `cs.CV`, `cs.RO` 최신 논문과 2022년 이후 공식 카테고리별 연간 등록 수 수집
...
- 분야·세부 주제 필터, 한국어 핵심 요약, 중요도·키워드 중심의 카드 뉴스와 기기별 읽음·별표 관리
...
카드의 한국어 요약은 별도 유료 API 없이 제목·초록의 연구 신호와 계층형 분류를 조합해 생성합니다. 따라서 번역문이 아니라 연구 분야, 핵심 접근법과 평가 목적을 빠르게 파악하기 위한 한국어 다이제스트입니다. 연도별 통계는 arXiv의 공식 연도별 카테고리 목록에 표시되는 등록 수를 사용합니다. 현재 연도는 연중 누적이며 교차 등록 논문은 여러 카테고리에 중복 집계될 수 있습니다.
